...
// Runtime types live here (not in models.go) so they are NOT picked up by ormc.
// ormc generates codecs for every struct in models.go/model.go; DevTUI and
// TuiConfig reference non-codegen types (tabSection, ColorPalette, viewport.Model,
// func fields) and must stay out of models.go. Only wire/form models belong there.
